...
TEMP_PACKAGE_DIR="$(mktemp -d)/package"
${SCRIPTS_DIR}/create-package.sh "${BUILD_DIR}" "${TEMP_PACKAGE_DIR}" > /dev/null

VersionInfo=($("${TEMP_PACKAGE_DIR}/${APP_NAME}.exe" -v))
APP_VERSION=${VersionInfo[1]}

${SCRIPTS_DIR}/create-package.sh -z "${BUILD_DIR}" "${OUTPUT_DIR}"
